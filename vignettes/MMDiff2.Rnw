...
protein-DNA interactions. This is a crucial method for understanding
gene regulation and epigenetics. A common example is performing
ChIP-seq for a transcription factor to determine where on the genome
the factor binds. The resulting ChIP-seq peaks are typically \textit{narrow}
in their footprint and can be further queried for enriched motifs. Another
example is the determination of chromatin states by profiling histone
...
primary tissues. These projects have described the regulatory
landscape through the lens of ChIP-seq experiments and also established
a standard for comparisons with other experimental datasets.
...
cells. ChIP-seq analysis begins with calling peaks between a sample conjugated
with an antibody against a control with DNA input. Various computational
tools exist for this task for example \textit{MACS2}. In an other scenario
...
After regions/peaks are identified, a common analysis is to determine regions
which are differentially bound by the studied transcription factor, or which
show differential histone modifications. For simplicity, we will call these regions
differential regions.  Often this is treated as a read-count based test, and some
statistical packages such as \DESeq2 can be utilized for this task. ChIP-seq specific
tools such as \DBA also integrate count-based testing in their methods.
...
read density may be insignificant but the distribution of reads within the
peak may imply differential protein binding or the presence of protein
complexes. Detection of these differential peak shapes is not possible when
simplifying peaks to single values.

We therefore incorporate higher order ChIP-seq peak information into
a testing for differential binding by adapting kernel-based
statistical tests to ChIP-Seq data. In this update to \textit{MMDiff}, we
improve code stability, decrease runtime and provide an interactive
application for exploring peak profiles.
...
ExperimentData <- list(genome='BSgenome.Mmusculus.UCSC.mm10',
...
\MD requires position sorted and indexed \texttt{BAM} files of the sample and input libraries and a set of regions in GRanges format, for example \texttt{MACS2} called peaks. For this
vignette, we utilize the \texttt{MMDiffBamSubset} dataset that contains ChIP-seq
peaks and reads from Clouaire et al., Genes \& Dev. 2012.
...
We load the \texttt{MMDiffBamSubset} and specify an experimental genome, a
directory containing peaks and reads, and a \textit{csv} file that details
the paths to different samples. The \texttt{MetaData} object will be called
upon later for adding in TB binding motif and gene annotation information.
...
class and read the BAM files for all peaks in all conditions.
...
Note, that we are fetching the exact start and end positions of mapped fragments, not the coverage. In the case of single-end reads, the left most positions of fragments mapping to the positive strands are stored in a list called Left.p, and the right most positions of fragments mapping to the negative strands are stored in Right.n. For paired-end reads Right.p and Left.n are additionally kept.

The next step in the pipeline is to estimate the fragment centers using
information from the forward and reverse strands. For paired-end reads, we can compute the exact Center position for each fragment, single-strand libraries we have to estimate the Centers. To improve on resolution, relative to coverage-based approach, we will go on using the (estimated) Center positions (see ref Gomes et al. Genome Research, 2014).
...
are computed using the read information. The step is essential for plotting regions; it is not essential in order determine differentially shaped regions. Two parameters as used to compute histograms: \texttt{bin.length}, which determines the smoothing parameter over the set of reads (Default: 20) and the \\{whichPos}, which determines the fragment position from which histograms should be computed: 'Left.p': left ends of fragments mapping to positive strand, 'Right.n' right ends of fragments mapping to negative strands, 'Center': Fragment centers for all positions.
...
plotPeak(MMD, Peak.id='6', plot.input = FALSE, whichPos="Center")
@
...
and Pax5 could be plotted. Integration of the motifs is seamless into
\texttt{plotPeak}:

<<peakplot2,message=FALSE,fig.width=7, fig.height=4, fig.align='center'>>=
plotPeak(MMD, Peak.id='6', NormMethod=NULL,plot.input = FALSE,whichPos="Center",
  Motifs=motifs,Motifcutoff="80%")
@
\subsubsection{Gene Annotation}

In addition to plotting motifs of interest, gene annotations can also be
plotted in a fully integrated plot with the ChIP-seq peak by providing
a genomicRanges object to the \texttt{anno} parameter.

<<peakplot3,message=FALSE,fig.width=7, fig.height=5, fig.align='center'>>=
data("mm9-Genes")
names(GR) <- GR$tx_name
GR <- list(UCSCKnownGenes = GR)
plotPeak(MMD, Peak.id='6', NormMethod=NULL,plot.input = FALSE,
...
performed by using a kernel-based test that calculates a distance between
treatment conditions and within conditions. The \texttt{compDists} function
...
1. between groups and 2. within groups. Differentially shaped peaks
are highlighted in red.

\subsection{Analyzing Results}
<<>>=
plotDists(MMD, dist.method='MMD',which.contrast=1,
...
plotPeak(MMD, Peak.id=Peak.ids[1], NormMethod=NULL,plot.input = FALSE,
         whichPos="Center",Motifs=motifs, anno=GR,which.contrast = 1)
dev.off()
plotDISTS4Peak(MMD,Peak.id=Peak.ids[1],dist.method='MMD',
               which.contrast=1,Zoom=TRUE)
...
To compare against the \DBA~package, we downloaded aligned BAM files
